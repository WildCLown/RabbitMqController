...
func main() { // Windows
	...
	numOfClients := 25 // for experimental purposes
...
func (c *Client) configureRabbitMQ() {
	...
	c.Conn, err = amqp.Dial("amqp://guest:guest@localhost:5672/") // local
	//c.Conn, err = amqp.Dial("amqp://nsr:nsr@localhost:5672/") // Docker
	//c.Conn, err = amqp.Dial("amqp://guest:guest@localhost:5672/") // Docker
	//c.Conn, err = amqp.Dial("amqp://guest:guest@192.168.0.10:5672/") // Docker
	//c.RepConn, err = amqp.Dial("amqp://nsr:nsr@localhost:5672/") // Docker
